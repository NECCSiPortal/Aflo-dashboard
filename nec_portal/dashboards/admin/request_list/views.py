...
from nec_portal.dashboards.admin.request_list \
    import constants
from nec_portal.dashboards.admin.request_list \
...
class IndexView(tables.DataTableView):
    """Index view class"""
    table_class = ticket_list_tables.RequestTable
    template_name = constants.TICKET_LIST_INDEX_VIEW_TEMPLATE
    page_title = _("Work Requests")
    ...
    def get_data(self):
        ...
        prev_marker = self.request.GET.get(
            ticket_list_tables.RequestTable._meta.prev_pagination_param, None)
        ...
        if prev_marker is not None:
            ...
            marker = self.request.GET.get(
                ticket_list_tables.RequestTable._meta.pagination_param, None)
        ...
        try:
            ...
            for ticket_row in ticket_list:
                ...
                template_contents = \
                    ticket_utils.get_ticket_template_contents(
                        self.request,
                        ticket_template_id)
                ...
                # Create View Rows
                service = TicketService(ticket_row.tenant_name,
    ...
    def get_filters(self):
        ...
        if filter_field and filter_string:
            try:
                if filter_field in ['tenant_id']:
                    filters[filter_field] = \
                        self._get_project_id(filter_string)
                elif filter_field in ['owner_at']:
                    if ticket_utils.is_date(filter_field, filter_string):
                        ...
                        filters['owner_at_from'] = \
                            ticket_utils.get_datetime_from(filter_string,
                                                           tzinfo)
                        filters['owner_at_to'] = \
                            ticket_utils.get_datetime_to(filter_string,
                                                         tzinfo)
