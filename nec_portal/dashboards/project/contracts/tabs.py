...
class OverviewTab(tabs.Tab):
    name = _("Overview")
    slug = "overview"
    template_name = "project/contracts/_detail.html"
    ...
    def get_context_data(self, request):
        ...
        base_url = ''
        if catalogs['ticket_info'] == '':
            pass
        elif catalogs['ticket_info']['cancelling_template'] is not None:
            cancel_info = catalogs['ticket_info']['cancelling_template']
            url = 'horizon:project:contracts:wf_engine_create:index'
            base_url = reverse(url,
                               args=[cancel_info['id'], contract.contract_id])
        return {"contract": contract,
                "ticket_list": ticket_catalog_list,
                "ticket_info": base_url}
